...
public class LoanAmountEstimatorSimulacionApi {
	...
	public okhttp3.Call getLAEByPersonCall(String xApiKey, PeticionPersona request,
			final ProgressResponseBody.ProgressListener progressListener,
			final ProgressRequestBody.ProgressRequestListener progressRequestListener) throws ApiException {
		Object localVarPostBody = request;
	...
	private okhttp3.Call getLAEByPersonValidateBeforeCall(String xApiKey, PeticionPersona request,
			final ProgressResponseBody.ProgressListener progressListener,
			final ProgressRequestBody.ProgressRequestListener progressRequestListener) throws ApiException {
		if (xApiKey == null) {
		...
		okhttp3.Call call = getLAEByPersonCall(xApiKey, request, progressListener, progressRequestListener);
		return call;
	}

	public Respuesta getLAEByPerson(String xApiKey, PeticionPersona request) throws ApiException {
		ApiResponse<Respuesta> resp = getLAEByPersonWithHttpInfo(xApiKey, request);
		return resp.getData();
	}

	public ApiResponse<Respuesta> getLAEByPersonWithHttpInfo(String xApiKey, PeticionPersona request)
			throws ApiException {
		okhttp3.Call call = getLAEByPersonValidateBeforeCall(xApiKey, request, null, null);
		Type localVarReturnType = new TypeToken<Respuesta>() {
		}.getType();
		return apiClient.execute(call, localVarReturnType);
	}
